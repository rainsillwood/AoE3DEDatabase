...
async function getTechs(isAll, iArray) {
  ...
  if (isAction || isAll) {
    ...
    appendNode('<table class="infoTable" id="' + tableName + '"></table>', 'databox', 'div');
    ...
    for (let i in oArray) {
      if (!oArray[i].isNull) {
        ...
        let oNode = getIcon(oArray[i]) + '<td>' + oArray[i]['@name'] + '</td>' + '<td>' + getRuby(oArray[i].displayname, oArray[i]['@name']) + '</td>' + '<td>科技</td>' + '<td>' + oArray[i].rollovertext + '</td>' + '<td class="effect"><div>' + stringEffect + '</div></td>';
  ...
  if (isAction || isAll) {
    ...
    for (let i in oArray) {
      if (!oArray[i].isNull) {
        ...
        let oNode = getIcon(oArray[i]) + '<td>' + oArray[i]['@name'] + '</td>' + '<td>' + getRuby(oArray[i].displayname, oArray[i]['@name']) + '</td>' + '<td>单位</td>' + '<td>' + oArray[i].rollovertext + '</td>' + '<td class="effect"><div>' + stringAttribute + '</div></td>';
        appendNode(oNode, tableName, 'tr');
        let oString =
          oArray[i]['@name'] +
          '\t' +
...
async function getNuggets(isAll, iArray) {
  ...
  if (isAction || isAll) {
    ...
    for (let i in oArray) {
      ...
      let oNode = getIcon(oArray[i]) + '<td>' + oArray[i].name + '</td>' + '<td>' + getRuby(nuggetUint.displayname, nuggetUint['@name']) + '</td>' + '<td>宝藏</td>' + '<td>' + oArray[i].rolloverstring + '</td>' + '<td class="effect"><div>' + stringEffects + '</div></td>';
      appendNode(oNode, tableName, 'tr');
      let oString =
        oArray[i].name +
        '\t' +
...
async function searchInfo(isFuzzy) {
  ...
  let tableName = 'info';
  appendNode('<table class="infoTable" id="' + tableName + '"></table>', 'databox', 'div');
  appendNode('<th class="icon">查询字段</th><th class="icon">图标</th><th class="name">调用名</th><th class="local">中文名</th><th class="type">类型</th><th class="desc">描述</th><th>效果</th>', 'info', 'tr');
  let iArray = getInner('input');
  for (let i in iArray) {
    let protoArray = [];
    let techArray = [];
    if (document.getElementById('getProto').checked) {
      protoArray = await getArray('proto', iArray[i], 'local', isFuzzy);
    }
    if (document.getElementById('getTech').checked) {
      techArray = await getArray('techtree', iArray[i], 'local', isFuzzy);
    }
